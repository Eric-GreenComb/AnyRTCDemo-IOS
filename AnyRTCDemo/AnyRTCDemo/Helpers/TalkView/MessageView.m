...
- (id)initWithFrame:(CGRect)frame withDic:(NSDictionary*)dict
{
    if (self = [super initWithFrame:frame]) {
        ...
        [self performSelector:@selector(clearEvent) withObject:nil afterDelay:8];
        // 头像
        ...
        CGSize  size = [textLabel preferredSizeWithMaxWidth:self.bounds.size.width -38];
        textLabel.numberOfLines = 0;
        textLabel.font = [UIFont systemFontOfSize:14.0f];
        textLabel.delegate = self;
        textLabel.textAlignment = NSTextAlignmentLeft;
        textLabel.frame = CGRectMake(CGRectGetMaxX(headImageView.frame)+8, CGRectGetMaxY(userNameLabel.frame), size.width, size.height);
