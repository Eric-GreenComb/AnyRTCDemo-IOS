...
- (void)receiveMessageView:(NSString*)str withUser:(NSString*)userID withHeadPath:(NSString*)iconPath
{
   // NSString *string = [ConvertToCommonEmoticonsHelper convertToSystemEmoticons:str];
    NSDictionary *dict;
    if (iconPath) {
        dict = [NSDictionary dictionaryWithObjectsAndKeys:str,@"content", userID,@"userID", iconPath, @"icon",nil];
    }else{
        dict = [NSDictionary dictionaryWithObjectsAndKeys:str,@"content", userID,@"userID", nil];
    }
    CGSize size = [self sizeWithNeiRong:dict];
    
    MessageView *message1 = [[MessageView alloc] initWithFrame:CGRectMake(10, self.bounds.size.height, size.width, size.height) withDic:dict];
...
{
    NSLog(@"sendMessageView");
   // NSString *willSendText = [ConvertToCommonEmoticonsHelper convertToCommonEmoticons:str];
   // NSDictionary *dict = [NSDictionary dictionaryWithObjectsAndKeys:str,@"content", userID,@"userID",[ParameterSettings settings]._iconPath,@"icon", nil];
        ...
        @synchronized(_dataArr){
            ...
            for (int i= allnum; i>=0; i--) {
                if (i==allnum) {
                    ...
                    message.frame = CGRectMake(bottonHeigthOrLeftAligain,self.bounds.size.height-( bottonHeigthOrLeftAligain+message.bounds.size.height), message.bounds.size.width, message.bounds.size.height);
                }else{
                    MessageView *lastMessage = [_dataArr objectAtIndex:i+1];
                    MessageView *newMessage = [_dataArr objectAtIndex:i];
                    newMessage.frame = CGRectMake(bottonHeigthOrLeftAligain, CGRectGetMinY(lastMessage.frame)-messagespace -newMessage.bounds.size.height, newMessage.bounds.size.width, newMessage.bounds.size.height);
                }
            }
        }
    }];
